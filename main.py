...
from modules.utils_glue import (compute_metrics, convert_examples_to_features,
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                inputs = {'input_ids':      batch[0],
                          ...
                          'labels':         batch[3]}
                outputs = model(**inputs)
                #print('model ouputs: {} '.format(outputs))
...
def predict(args, model, tokenizer, prefix=""):
    ...
    results = {}
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_predict_data(args, eval_task, tokenizer, evaluate=True)
...
def load_predict_data(args, task, tokenizer, evaluate=False):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
...
def main():
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            (result, predict_dict) = predict(args, model, tokenizer, prefix=global_step)
            result = dict((k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)
